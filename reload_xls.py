...
def detect_img(yolo):  
    ...
    while True:
        try:  
            image = Image.open('E://cry//keras-yolo3//罚篮处理//视频文件//4//%s.jpg'%i)
        except:  
            print('Open Error! over!')
            break
        else:  
            r_image = yolo.detect_image(image,i)
            #r_image.show()
            r_image.save('E://cry//keras-yolo3//罚篮处理//视频文件//4_result//%s.jpg'%i)
            i=i+1
    wb.save('E://cry//keras-yolo3//罚篮处理//视频文件//4.xls')
    yolo.close_session()  
...
if __name__ == '__main__':
    ...
    wb = xlwt.Workbook()
    ws = wb.add_sheet('basketball')
    ws2 = wb.add_sheet('B Test Sheet')
    ws3 = wb.add_sheet('C Test Sheet')
    ws4 = wb.add_sheet('hoop')
    ws5 = wb.add_sheet('E Test Sheet')
    detect_img(YOLO())